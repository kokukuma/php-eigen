/*
  +----------------------------------------------------------------------+
  | PHP Version 5                                                        |
  +----------------------------------------------------------------------+
  | Copyright (c) 1997-2011 The PHP Group                                |
  +----------------------------------------------------------------------+
  | This source file is subject to version 3.01 of the PHP license,      |
  | that is bundled with this package in the file LICENSE, and is        |
  | available through the world-wide-web at the following url:           |
  | http://www.php.net/license/3_01.txt                                  |
  | If you did not receive a copy of the PHP license and are unable to   |
  | obtain it through the world-wide-web, please send a note to          |
  | license@php.net so we can mail you a copy immediately.               |
  +----------------------------------------------------------------------+
  | Author:                                                              |
  +----------------------------------------------------------------------+
*/

/* $Id: header 310447 2011-04-23 21:14:10Z bjori $ */

#ifndef PHP_EIGEN_H
#define PHP_EIGEN_H

//extern "C" {

extern zend_module_entry eigen_module_entry;
#define phpext_eigen_ptr &eigen_module_entry

#ifdef PHP_WIN32
#	define PHP_EIGEN_API __declspec(dllexport)
#elif defined(__GNUC__) && __GNUC__ >= 4
#	define PHP_EIGEN_API __attribute__ ((visibility("default")))
#else
#	define PHP_EIGEN_API
#endif

extern "C"{
	#ifdef ZTS
	#include "TSRM.h"
	#endif
}
PHP_MINIT_FUNCTION(eigen);
PHP_MSHUTDOWN_FUNCTION(eigen);
PHP_RINIT_FUNCTION(eigen);
PHP_RSHUTDOWN_FUNCTION(eigen);
PHP_MINFO_FUNCTION(eigen);

PHP_FUNCTION(confirm_eigen_compiled);	/* For testing, remove later. */
// helloworld
PHP_FUNCTION(eigenworld);
// Eigen Function type 1 
PHP_FUNCTION(Eigen2);
PHP_METHOD(Eigen2, Singular_Value_Decomposition);
PHP_METHOD(Eigen2, Inner_Product);
static zend_class_entry *eigen_ce; 
// Eigen Function type 2 
PHP_FUNCTION(Eigen_Singular_Value_Decomposition);
PHP_FUNCTION(Eigen_Feature_Vector);
PHP_FUNCTION(Eigen_Inner_Product);
PHP_FUNCTION(Eigen_Vector_Length);
PHP_FUNCTION(Eigen_COS);


/* 
  	Declare any global variables you may need between the BEGIN
	and END macros here:     

ZEND_BEGIN_MODULE_GLOBALS(eigen)
	long  global_value;
	char *global_string;
ZEND_END_MODULE_GLOBALS(eigen)
*/

/* In every utility function you add that needs to use variables 
   in php_eigen_globals, call TSRMLS_FETCH(); after declaring other 
   variables used by that function, or better yet, pass in TSRMLS_CC
   after the last function argument and declare your utility function
   with TSRMLS_DC after the last declared argument.  Always refer to
   the globals in your function as EIGEN_G(variable).  You are 
   encouraged to rename these macros something shorter, see
   examples in any other php module directory.
*/

#ifdef ZTS
#define EIGEN_G(v) TSRMG(eigen_globals_id, zend_eigen_globals *, v)
#else
#define EIGEN_G(v) (eigen_globals.v)
#endif

//} /* end of extern "C" */

#endif	/* PHP_EIGEN_H */


/*
 * Local variables:
 * tab-width: 4
 * c-basic-offset: 4
 * End:
 * vim600: noet sw=4 ts=4 fdm=marker
 * vim<600: noet sw=4 ts=4
 */
